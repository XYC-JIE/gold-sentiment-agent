...
4. 下载该次运行的 artifacts（`charts-<run_id>`，保留 7 天）确认三张图的中文正常 —— 2026-09-19 已给 workflow 补上 `actions/upload-artifact`（带 `if: always()`，好让流水线失败时也能拿到已生成的那几张图来归因）。**补之前这条在 CI 上做不到**，只能靠字体护栏做间接证据。若图上出现方框，去查 `_pick_available_cjk_font` 里 `_has_cjk_glyphs` 那道校验的调用点是否还在（删除它是静默的，用例会拦住）
5. 日志里若有 `失败源 N 个`，记下是哪些

**另外必须单独确认「金价」这一项**——它是设计文档 §5.4 点名的分析价值支点。

> **本段已更新（2026-09-19）**：原文写的是 stooq 的拦截风险，**该前提已过期**。金价源在 **Task 5b 就换成了新浪财经** `https://hq.sinajs.cn/list=hf_XAU`（stooq 在本机与 CI 两个环境都长期失效）。当前 `fetch_latest()` 取 `fields[0]`（最新价），日期用正则 `_DATE_RE.findall(payload)[-1]` 取，**不存在**原文警告的那种"取响应顺序最后一条、可能静默返回最旧价"的排序假设。所以下面要查的是**新浪这条链路**。

具体查三件事：

1. 卡片上**没有**出现「现货黄金」这条失败告警
2. `data/daily_index.csv` 最新一行的 `gold_close` **非空**
3. `charts/` 下有 `sentiment_vs_gold.png`，且打开看两条线都画出来了

新浪链路唯一已知的硬约束：**请求必须带 `Referer: https://finance.sina.com.cn`，否则返回 HTTP 403**（响应体是 9 字节的 `Forbidden`，`raise_for_status()` 直接抛 `HTTPError`，**不是**「返回空内容」）。所以 `GoldPriceCollector` 里的 `SINA_HEADERS` 不能删；改这个采集器时务必保留。详见 `docs/信源核验记录.md` 的 2026-09-19 小节。

若三条中有任一条不满足，说明新浪在 CI 上也被拒了。应对：再换一个金价源（Yahoo `GC=F` 之类），改 `GoldPriceCollector` 的解析部分，改完记进 `docs/信源核验记录.md`。**历史行情随处可得，中间断的那几天可以事后回填**，所以这不紧急，但不能不查。

> **实测记录（2026-09-19，CI run #4）**：第 1、2 条**已满足**（卡片无金价告警、`gold_close=4378.29`）。第 3 条在 run #4 时只有间接证据（「校验中文字体可用」步骤通过 ⇒ 出图路径中文字体链路正常）；`actions/upload-artifact` 已于同日补进 workflow，**下一次运行起可直接看图**。
