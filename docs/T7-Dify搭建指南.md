...
4. **安全设置这一步很关键**：勾选 **自定义关键词**，填：
   ...
   > 为什么填这个：飞书要求机器人至少有一种安全设置。选了「关键词」之后，**消息里必须包含这个词才发得出去**，否则返回 `19024 Key Words Not Found`。我们生成的卡片标题固定是 `📊 每日日报 · 日期`，降级告警正文也是同样开头，所以这个词一定命中。
   >
   > ⚠️ **这里的词必须和代码里的 `FEISHU_KEYWORD`（`src/notifier.py`）一字不差。** 改了一边忘了另一边，第二天早上的推送会**静默全挂**——飞书拒收，但日志里只有一行错误码。测试里有一条用例专门钉住这个。
   >
...
curl -s -X POST "你的webhook地址" \
  ...
  -d '{"msg_type":"text","content":{"text":"每日日报 测试消息"}}'
```

飞书群里应该立刻出现这条消息。返回 `{"code":0,...}` 就是成功。

> 注意 `每日日报` 这四个字不能省，否则会被关键词安全设置拦下，返回错误码。
...
3. 点右上角 **运行/预览**，按下面的固定测试数据跑一次。跑不通先别导出，告诉我报什么错。
...
| # | 内容 | 期望 event_type | 期望 direction | 期望 relevant |
|---|---|---|---|---|
| 1 | OpenAI 发布新模型 | 其他 | 中性 | **false** |
| 2 | 谷歌开源框架 | 其他 | 中性 | **false** |
| 3 | 美联储官员放鸽 | 货币政策 | **利多金银** | **true** |
| 4 | CPI 超预期 | 通胀 | **利空金银** | **true** |
| 5 | 黄金 ETF 净流入 | 其他 | 利多金银 | **true** |

通用核对项：数组长度**正好 5**、顺序与输入一致、`event_type` 全在枚举内、`strength` 是 1–5 的**整数**（不是字符串）。

> **第 3、4 条是关键**：降息预期该判利多、通胀超预期该判利空。**方向判反了要立刻告诉我**——那是模型逻辑问题，不是配置问题。
...
|---|---|
| `ai_news` | 2 条 |
| `gold_news` | 3 条，按 `strength` 从高到低 |
| `gold_conclusion` | 一句中文，倾向**偏多**（2 多 1 空） |
| `calendar` | 空数组或含 CPI，两种都对 |
| 编造检查 | **`url` 必须来自输入**，不能出现输入里没有的链接 |

**跑通后必做：点右上角「发布」/「更新」。** 不发布的话 API 调的还是旧版本，你会以为配置没生效。
...
| 现象 | 多半是 |
|---|---|
| 飞书返回 `code` 非 0，提示关键词不匹配 | 消息里没带关键词。检查飞书后台的关键词与代码里的 `FEISHU_KEYWORD` 是否一字不差 |
